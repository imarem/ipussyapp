...
//		getSupportActionBar().setDisplayHomeAsUpEnabled(true);
	...
	private static class SampleView extends View {
		...
		private void initVariables(int time, PussyModel p) {
			...
			countDownTimer = new CountDownTimer(totalTime + 1300, 500) {
				...
				public void onTick(long millisUntilFinished) {
					if (counterHalf) {
						// cada mitat de segon realitzem una predicci� del
		...
		public void drawGameSequence(Canvas canvas) {
			...
			// colors interns en funci� de la moel.
			offset = 0;
...
//				float bottom = unitatHeight  + (unitatHeight / 2) -2;// - unitatHeight / 8;
		...
		/**
		 * M�tode que dibuixa la sequencia de velocitats realitzada per
		 * l'usuari-
		 * 
		 * @param canvas
		 *            el canvas
...
//				float bottom = unitatHeight * 2 -2;
		...
		/**
		 * M�tode que pinta la zona interna, el moviment amb dos dits i el
		 * goteix.
		 * 
		 * @param canvas
		 *            el canvas
		 ...
		 * @return la regi� interior.
		 */
		private Region drawInsideZone(Canvas canvas, Region regionVulva) {
			// XOX INTERIOR
			// creem xox interior en funci� de les coordenandes dels dit dret i
			// esquerra.
			Path pathXox = null;
		...
		/**
		 * M�tode que dibuixa la zona de fora.
		 * 
		 * @param canvas
		 *            el canvas.
		...
		private Region drawOutsideZone(Canvas canvas) {
			...
			// creem regi� per al path per a limitar la obertura del pussy
			return Utils.createRegionByPath(pathVulva);
		}

		/**
		 * M�tode que crea i limita la regio de vibraci�
		 * 
		 * @param canvas
		 *            el canvas.
		 */
		...
		/**
		 * M�tode que a partir d'un coordenanda permet actvar el joc si toques
		 * el clitoris.
		 * 
		 * @param x
		 *            la x.
		...
		private void isInitGame(int x, int y) {
			if (flagInit
				...
				// primer moviment de cal�a temps
				makeCalcaTimerAnimation(1, 2);
			}
		}

		/**
		 * M�tode que inicialitza l'objecte velocity traker.
		 * 
		 * @param event
		 *            l'event touch.
		 */
		...
		/**
		 * M�tode on s'avalua la velocitat (pixels/segon) i es realitza el
		...
		/**
		 * M�tode que permet evaluar la velocitat de l'usuari durant cada segon,
		...
		/**
		 * M�tode que crea i mou la cal�a dins la barra de potencia.
		 * 
		 * @param inici
		 *            l'inici.
		 * @param fi
		...
		/**
		 * M�tode que crea i mou la cal�a a la barra de temps.
		 * 
		 * @param xFromDelta
		 *            l'inici.
		 * @param xToDelta
		...
		/**
		 * M�tode que compara la velocitat esperada, amb la del usuari i permet
		 * evaluar.
		 */
